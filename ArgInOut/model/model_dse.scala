...
object AppRuntimeModel_dse extends App {
  ...
  override def main(args: Array[String]): Unit = {
    begin("/local/ssd/home/mattfel/sp_fixes/spatial/./gen/VCS/ArgInOut///results_dse")
    ...
    emit(s"[dse] Structure for app ArgInOut")
    allTuneParams.foreach{tuneTo => 
        tuneParams = tuneTo
        root.printStructure()
        root.execute()
        emit(s"[dse] Runtime results for app ArgInOut")
        root.printResults()
        ...
        emit(s"[dse] Total Cycles for App ArgInOut: ${root.totalCycles()}")
    }
    end()
  }
}
